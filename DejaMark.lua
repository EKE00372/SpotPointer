...
_G["BINDING_NAME_CLICK DejaMark:LeftButton"] = "Hold Show Markers"
...
for index = 1, 9 do
	-- Lua starts tables at 1 isntead of 0... so below I do lots of equating 9 to 0 as an index...
	-- Button = CreateFrame("Button", "DejaMarkIconButton"..index, self);
	-- Use "SecureActionButtonTemplate" if using macros

	Button = CreateFrame("Button", "DejaMarkIconButton"..index, DejaMarkFrame, "SecureActionButtonTemplate")
	Button:SetWidth(32)
	Button:SetHeight(32)
	Button:SetID(index)
	...
	if index == 9 then
		...
		-- Blizzard specific function that centers sprite texture via index but doesn't include clearing all marks
	...
	--Comment out if using macros
	--Button:SetScript("OnClick", DejaMarkButton_OnClick) 
